...
| 모노토닉 스택 | 각 원소마다 "**나보다 먼저 나오는 더 큰/작은 값**"을 묻는다. 완탐하면 O(n²)이고 n ≥ 10⁵ | [LeetCode 739](https://leetcode.com/problems/daily-temperatures/) | [코드](https://github.com/userri/Algorithm/blob/main/LeetCode/0739-daily-temperatures/0739-daily-temperatures.java) |
...
### ⑪ 모노토닉 스택 — 관점을 뒤집으면 대기열이 보인다

**판정 신호:** 각 원소마다 "나보다 먼저 나오는 더 큰(작은) 값" 을 묻는다. 완탐이 O(n²)인데 n 이 크다.

발상은 **누가 답을 찾는가를 뒤집는 것** 하나다.

```
완탐   : i 입장에서 나보다 큰 놈을 앞으로 찾아간다   → 매번 다시 훑음, O(n²)
뒤집기 : j 에 도착했을 때, j 가 해결해 줄 수 있는 과거 놈들은 누구인가
```

j 가 등장하면 **아직 답을 못 받은 놈들이 한꺼번에** 해결된다. 그러니 "아직 답을 못 받은 놈들"만 들고 다니면 된다.

**왜 스택인가 — 순서가 공짜로 보장되기 때문이다.**
대기 중에 75 다음 71 이 들어왔다면, 75 를 깨는 값은 71 도 반드시 깬다.
즉 **71 이 75 보다 먼저 해결된다.** 대기열은 항상 내림차순이고, 그래서 앞을 뒤질 일이 없다 → 뒤에서만 꺼낸다 → 스택.

**복잡도:** 각 원소가 한 번 들어가고 한 번 나온다 → `while` 이 중첩돼 보여도 **O(n)**.

**짜기 전에 세 줄 적는다** (여기서 W1 이 난다)
```
1. 스택에 값? 인덱스?   → 답이 j - i 면 인덱스. 값은 temp[idx] 로 언제든 꺼낸다
2. > 인가 >= 인가       → "더 따뜻한"이면 >. 같은 값을 밀어낼지 문제 문구로 결정
3. 끝나고 남은 놈들은?  → 아래
```

**남은 놈 뒷정리는 초기값으로 없앤다.**
루프가 끝났는데 스택에 남았다 = 끝까지 못 깨졌다 = **답 없음**.
그러니 "답 없음"을 처음부터 깔아두면 뒷정리 코드가 사라진다.

```
739 (없으면 0)  → new int[n] 이 이미 0. 아무것도 안 해도 됨
496 (없으면 -1) → Arrays.fill(result, -1) 로 깔고 시작
```
739 에서 뒷정리가 없는 건 **답이 0이라서 생긴 우연**이다. 골격으로 재사용할 때 이걸 잊으면 -1 문제에서 깨진다.

**실제 사례 (739):** 브루트포스 때 붙인 `max` 가지치기가 스택 풀이에도 그대로 남았다.
① 최악 O(n²)를 못 줄이는 최적화였고 ② 스택 풀이에선 `result` 가 이미 0이라 아무 일도 안 한다.
접근을 갈아탈 때 **이전 접근의 잔해도 같이 나가야 한다** → ⑩ 과 같은 뿌리.
같은 커밋에 `temp[temp.length-1] = 0;` 도 있었다 — `temp` 는 입력 배열의 별칭이라 **입력을 훼손**했고, 하려던 일(`result`)도 안 됐다 → W1.
